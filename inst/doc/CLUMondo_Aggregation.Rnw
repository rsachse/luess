...
Each mosaic consists of one of the five basic landuse classes: trees, buildt up area, bare area, cropland or pasture. 
...
reserved for potential natural vegetation. Therefore, I summed up tree area and bare area for the area of potential natural vegetation.
After aggregation for LPJmL we have the three classes: natveg, cropland and pasture remaining. Natveg and pasture translates directly to LPJmL, 
while cropland needs to be translated to 13 different crop functional types (CFTs) later.

To disentangle the underlying
real land use classes per grid cell, the generic function \texttt{aggregateMosaics()} or a LPJmL specific 
...
\subsection{A Method based on an existing and most recent land use map}
\ldots{} not implemented yet.
