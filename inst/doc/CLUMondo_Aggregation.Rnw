...
\SweaveOpts{concordance=TRUE, pdf=FALSE, png=TRUE, eps=FALSE, width=7, height=6, res=100}
...
@
\section{Final remarks about changes in latest package versions}
\subsection{resample\_grid() output format}
