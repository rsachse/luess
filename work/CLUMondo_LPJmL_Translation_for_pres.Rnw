...
par(mfrow=c(3,1))
png("croplpj.png", width=4000, height=3000, res=600)
gridPlot(cropLPJ, main="LPJmL cropland without pasture 2000",col=greens(50), addcountries=TRUE, lwd=1)
dev.off() 

png("cropclu.png", width=4000, height=3000, res=600)
gridPlot(cropCLU, zlim=c(0,1), main="CLUMondo cropland 2000", col=greens(50), addcountries=TRUE, lwd=1)
dev.off() 

png("cropdelta00.png", width=4000, height=3000, res=600)
gridPlot(cropDelta, main=expression(paste(Delta,"cropland = ",CLUMondo-LPJmL)), zlim=c(-1,1),col=myPalette(100),addcountries=TRUE)
dev.off() 
...
#absrange <- max(abs(c(min(cropDelta), max(cropDelta))))
par(mfrow=c(3,1))
png("pasturelpj.png", width=4000, height=3000, res=600)
gridPlot(pastLPJ, main="LPJmL Pasture", col=greens(50), addcountries=TRUE)
dev.off()
png("pastureclu.png", width=4000, height=3000, res=600)
gridPlot(pastCLU, zlim=c(0,1), main="CLUMondo Pasture", col=greens(50), addcountries=TRUE)
dev.off()

png("pasturedelta00.png", width=4000, height=3000, res=600)
gridPlot(pastDelta, main=expression(paste(Delta,"pasture = ","CLUMondo-LPJ")), zlim=c(-1,1),col=myPalette(100), addcountries=TRUE)
dev.off()
...
png("cropdelta40.png", width=4000, height=3000, res=600)
#cropDelta2040 <- ifelse(cropDelta2040 > 0.4, 0.4, cropDelta2040)
#cropDelta2040 <- ifelse(cropDelta2040 < -0.4, -0.4, cropDelta2040)
redblue <- colorRampPalette(brewer.pal(11,"RdBu"))
rb_raw <- redblue(101)
rb_pal <- colorRampPalette(rb_raw[c(1,10,15,50,85,90,101)])

gridPlot(cropDelta2040, main=expression(paste("OECD ",Delta,"cropland (2040-2000)")), zlim=c(-1,1),col=rev(rb_pal(101)), addcountries=TRUE)
dev.off()
